...
def read_and_extract_data_from_file(file_name: str, last_iteration=0) -> dict:
    """Unzip, read, and store data into pandas dataframe."""
    with _open_zipped_file(file_name=file_name) as file:
        ...
        for i, text_line in enumerate(file):
            ...
            sliced_text_line = _slice_text_data(text_line=text_line)
        ...
        print(f"Reached the end of file {file_name}.")
        return {
            "has_more": False,
            "extracted_data": dataframe
        }
...
def _slice_text_data(text_line):
    return {
...
if __name__ == "__main__":
    read_and_extract_data_from_file(
        file_name='COTAHIST_A1986.zip'
    )
